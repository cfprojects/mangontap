					...
					<cfelseif allPlugins[i].eventType EQ "asynch">
						...
						<cftry>
							<cfthread action="run" name="thread#i##randrange(1,10000)#" pluginId="#pluginId#" event="#arguments.event#">
								<cfset var thisPlugin = variables.plugins[thread.pluginId]>
								<cfset var logger = ""/>
								
								<cftry>
									<cfset thisPlugin.handleEvent(thread.event) />
									<cfcatch type="any">
										<cfset logger = tapGetLogger() />
										<cfset logger.logObject("error",cfcatch, "Error while calling plugin #thread.pluginId#") />
									</cfcatch>
								</cftry>
							</cfthread>
